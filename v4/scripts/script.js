...
if (screen.width <= 699) {
  window.location.href = "https://temp.dog/v4/mobile";
}
...
// force the background and portfolio webms onto the same frame:
// wait until both are loaded, then start them together from frame 0
function syncVideos() {
  video.currentTime = 0;
  portfolio.currentTime = 0;
  video.play();
  portfolio.play();
}
...
Promise.all([whenLoaded(video), whenLoaded(portfolio)]).then(syncVideos);